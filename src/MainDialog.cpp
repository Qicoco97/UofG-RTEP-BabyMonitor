...
FrameCallback *camCallback = nullptr;
    ...
    , itsFps(itsSource ? itsSource->fps() : 0.0)
{
    ...
    connect(camCallback, &FrameCallback::frameReady, this, &MainDialog::onFrameReady, Qt::QueuedConnection);
    camera.registerCallback(camCallback);
    camera.start();
    if (itsSource && itsTransform) {
        ...
        camCallback = new FrameCallback(this);
  // 开始推流
...
void MainDialog::onFrameReady(const QImage &image) {
    // 将接收到的新帧图像显示到界面上的视频控件
    ui->displayWidget->showImage(image);  
    // 或者 displayWidget->setPixmap(QPixmap::fromImage(image)); 视具体DisplayWidget实现而定
}
